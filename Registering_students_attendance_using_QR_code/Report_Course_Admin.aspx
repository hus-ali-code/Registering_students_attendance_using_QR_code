<%@ Page Title="" Language="C#" MasterPageFile="~/admin.Master" AutoEventWireup="true" CodeBehind="Report_Course_Admin.aspx.cs" Inherits="Registering_students_attendance_using_QR_code.Report_Course_Admin" %>
...
<asp:Content ID="Content2" ContentPlaceHolderID="ContentPlaceHolder1" runat="server">
        ...
        <div class="div2">
            ...
            <div class="m-2">
                <%= Get_Dept(int.Parse(Request.QueryString["CourseID"])) %>
             </div>
            <i class="fa-solid fa-arrow-right fs-6"></i>
            <div class="m-2">
                <%= Get_Divsion(int.Parse(Request.QueryString["CourseID"])) %>
                </div>
            <i class="fa-solid fa-arrow-right fs-6"></i>
             <div class="m-2">
                <%= Request.QueryString["Cou_Name"] %>
                </div>
 ...
 <ItemTemplate>
        <tr style="font-size:15pt; font-weight:bold" class="text-center align-middle">
            <td><img src='<%# Eval("Photo") %>' style="width:100px; height:120px;" /></td>
          ...
          <td> <label id="Label2"><%# Get_Lectures_Attended_ByStudent(int.Parse(Eval("student_id").ToString())) %></label> من اصل <%# Get_Lectures_ForCourse() %> محاضرة</td>
          <td> <label id="Label3"><%# Get_Lectures_ForCourse() - Get_Lectures_Attended_ByStudent(int.Parse(Eval("student_id").ToString())) %></label></td>
          <td>

 <asp:Label runat="server"  id="Label4" 
                   ForeColor='<%# (Get_Lectures_Attended_ByStudent(int.Parse(Eval("student_id").ToString())) * 100.0f) / Get_Lectures_ForCourse() < 75 ? System.Drawing.Color.Red : System.Drawing.Color.LimeGreen %>'>
                <%#((Get_Lectures_Attended_ByStudent(int.Parse(Eval("student_id").ToString())) * 100.0f) / Get_Lectures_ForCourse()).ToString("F2") + "%"%>
            </asp:Label>
          </td>
